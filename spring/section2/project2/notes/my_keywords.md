# 1. What is NoUniqueBeanDefinitionException ? [->](./NoUniqueBeanDefinitionException.md#4-get-bean-by-name)
    * Learn what is type casting?

# 2. Different ways to name a Bean. [->](./CustomBeanNames.md) 
    1. @Bean(name=string)
    2. @Bean(value=string)
    3. @Bean(string)
...
# 3. chosing the 'Default' Bean [->](./PrimaryBean.md)
    1. @Primary

# 4. Modular config made easy with @Import [->](./Annotations/Import.md)

    1. ApplicationContext context = new AnnotationConfigApplicationContext(AppConfig.class, DatabaseConfig.class);
    2. @Import({})

# 5. Bean creation the Easy way
...
# 7. (Lec 20) @Bean vs @Component Therory

# 8. (Lec 19) Spring's sterotypes - @Componet, @Service, @Repository, @Controller
...
# 9. (Lec 20) Method-Based Bean wiring -Theory.
...
# 10. (Lec 21) Method-Based wiring - practical [code](../src/main/java/com/eazybytes/methodBasedWiring/MethodBasedWiring.java)

# 11. (Lec 22) Autowiring - Field vs Setter Injection.
...
# 12. (Lec 23) Autowiring with @Primary